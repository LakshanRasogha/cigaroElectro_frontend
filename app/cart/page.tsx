...
const CartPage = () => {
  ...
  useEffect(() => {
    ...
    if (savedBag) {
      try { setCartItems(JSON.parse(savedBag)); } 
  ...
  if (isSyncing) return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#030303] gap-6">
      ...
      <p className="text-zinc-700 font-black text-[10px] uppercase tracking-[0.4em]">Initialize Bag...</p>
    </div>
  );
  ...
  return (
    <>
      <Navbar />
      <div ref={containerRef} className="bg-[#030303] min-h-screen pt-32 relative overflow-hidden selection:bg-[#D4AF37]/30 font-sans">
        ...
        <div className="max-w-7xl mx-auto px-6 pb-32 relative z-10">
          <header className="mb-16 cart-animate">
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className="text-[#D4AF37] font-bold text-[8px] uppercase tracking-[0.6em]">Terminal Bag 0.1</span>
            </div>
          ...
          <div className="grid lg:grid-cols-3 gap-12 items-start">
              ...
              <AnimatePresence mode="popLayout">
                {cartItems.length > 0 ? (
                  ...
                  >
                    ...
                    <p className="text-zinc-600 font-bold text-[10px] uppercase tracking-[0.4em] mb-10">No Assets Allocated</p>
                    <Link href="/collections" className="text-[#D4AF37] font-black text-[9px] uppercase tracking-widest border-b border-[#D4AF37]/20 pb-1">Start Discovery</Link>
                  </motion.div>
                )}
              </AnimatePresence>
            </div>
            ...
            <div className="cart-animate">
              <div className="sticky top-32 p-10 rounded-[3rem] bg-white/[0.01] border border-white/5 backdrop-blur-3xl shadow-2xl">
                <h2 className="text-xs font-serif tracking-[0.4em] text-white mb-10 uppercase">Order Summary</h2>
                ...
                <div className="space-y-6 mb-12">
                  ...
                  <div className="space-y-2">
                    <p className="text-[#D4AF37] text-[8px] font-bold uppercase tracking-[0.4em]">Total Dispatch</p>
                    <p className="text-4xl font-serif tracking-tighter text-white">Rs. {total.toLocaleString()}</p>
                  </div>
                </div>
  ...
  return (
    ...
    >
      ...
      <div className="w-full lg:w-40 aspect-square lg:h-44 rounded-2xl overflow-hidden bg-black shrink-0">
        <img src={item.image} className="w-full h-full object-cover opacity-60 group-hover:scale-105 transition-transform duration-700" alt={item.name} />
      </div>

      {/* Product Details */}
      <div className="flex-1 w-full text-center lg:text-left space-y-2 lg:space-y-4">
        <div>
          <h3 className="text-[10px] lg:text-xl font-serif text-white tracking-[0.1em] lg:tracking-widest uppercase truncate">{item.name}</h3>
          <p className="text-[7px] lg:text-xs text-zinc-500 italic font-light tracking-widest uppercase truncate">{item.flavor} {item.emoji}</p>
        </div>
        <p className="text-xs lg:text-2xl font-serif text-[#D4AF37] tracking-tighter">Rs. {item.price.toLocaleString()}</p>
      </div>
      ...
      <div className="flex lg:flex-col items-center justify-between w-full lg:w-auto gap-4 pt-2 lg:pt-0">
        <div className="flex items-center gap-3 lg:gap-5 bg-black/40 p-1.5 rounded-xl border border-white/5">
          <button onClick={() => onUpdate(item.cartId, -1)} className="p-1 text-zinc-500 hover:text-white transition-colors"><Minus size={10} /></button>
          <span className="text-[9px] font-black text-white w-3 text-center">{item.quantity}</span>
          <button onClick={() => onUpdate(item.cartId, 1)} className="p-1 text-zinc-500 hover:text-white transition-colors"><Plus size={10} /></button>
        </div>
...
const AddressModal = ({ isOpen, onClose, user, option, setOption, newAddress, setNewAddress, onConfirm, loading }: any) => {
    ...
    <div className="fixed inset-0 z-[120] flex items-center justify-center p-6">
      ...
      >
        <div className="flex justify-between items-center mb-10">
          <h2 className="text-xl font-serif tracking-[0.2em] text-white uppercase italic">Dispatch Zone.</h2>
        ...
        <div className="space-y-4">
          ...
          >
            <div className="p-3 rounded-xl bg-white/5 text-[#D4AF37]"><Home size={18} /></div>
            <div>
              <p className="text-[7px] font-bold uppercase tracking-widest text-zinc-500 mb-1">Stored Identity</p>
              <p className="text-[11px] text-white tracking-widest uppercase">{user?.address?.address || "No Stored Data"}</p>
            </div>
          </button>
          ...
          >
            <div className="p-3 rounded-xl bg-white/5 text-zinc-500"><MapPin size={18} /></div>
            <p className="text-[7px] font-bold uppercase tracking-widest text-zinc-600 mt-3">Manual Destination</p>
          </button>
          ...
          {option === 'new' && (
            <div className="space-y-3 pt-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <input placeholder="CITY" onChange={(e) => setNewAddress({...newAddress, city: e.target.value})} className="bg-white/5 border border-white/5 rounded-xl px-5 py-4 text-[9px] text-white tracking-widest uppercase outline-none placeholder:text-zinc-800" />
                <input placeholder="POSTAL" onChange={(e) => setNewAddress({...newAddress, postalCode: e.target.value})} className="bg-white/5 border border-white/5 rounded-xl px-5 py-4 text-[9px] text-white tracking-widest uppercase outline-none placeholder:text-zinc-800" />
              </div>
            </div>
          )}
          ...
          >
            {loading ? <Loader2 className="animate-spin" size={14} /> : "Initialize Synchronized Dispatch"}
          </motion.button>
        </div>
      </motion.div>
    </div>
