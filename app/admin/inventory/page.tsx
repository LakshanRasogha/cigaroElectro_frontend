...
const InventoryRow = ({
  ...
  return (
    <tr className='group hover:bg-slate-50/50 transition-colors border-b border-slate-50 last:border-0'>
      <td className='py-5 px-8'>
        <div className='flex items-center gap-4'>
          ...
          <div className='max-w-[200px]'>
            <p className='text-sm font-bold text-slate-900 mb-0.5 truncate'>
              {product.name}
            </p>
      ...
      <td className='py-5 px-4 text-xs font-bold text-slate-500 uppercase'>
        {product.category}
      </td>
      <td className='py-5 px-4 font-mono text-sm font-bold text-slate-700'>
        Rs. {product.basePrice?.toLocaleString()}
      </td>
      <td className='py-5 px-4 text-sm font-bold text-slate-900'>
        {totalStock.toLocaleString()} Units
      </td>
      <td className='py-5 px-4'>
        <div
          className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border text-[10px] font-black uppercase tracking-wider ${getStatusStyles(status)}`}
        >
          {status === "In Stock" ? (
    ...
    try {
      const payload = {
        ...
        variants: formData.variants.map((v) => ({
          ...
          variantImage: Array.isArray(v.variantImage)
            ? v.variantImage
            : [v.variantImage],
        })),
      };
        ...
        <div className='fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm'>
          <div className='bg-white w-full max-w-3xl max-h-[95vh] overflow-y-auto rounded-[2.5rem] shadow-2xl p-8 md:p-12 relative animate-in zoom-in-95 duration-200'>
            ...
            <h2 className='text-3xl font-bold text-slate-900 mb-8 tracking-tight'>
              {editingKey ? "Edit Product" : "New Entry"}
            </h2>
            ...
            <form onSubmit={handleFormSubmit} className='space-y-6'>
              ...
              <div className='grid grid-cols-2 gap-4'>
                <input
                  ...
                  className='w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl outline-none text-sm focus:bg-white'
                ...
                {formData.variants.map((v, idx) => (
                  ...
                  >
                    ...
                    <div className='grid grid-cols-12 gap-3 mb-4'>
                      <input
                        required
                        className='col-span-6 p-3 rounded-xl border border-slate-200 text-xs focus:bg-white'
                        placeholder='Flavor'
                        value={v.flavor}
                        onChange={(e) => {
                          const n = [...formData.variants];
                          n[idx].flavor = e.target.value;
                          setFormData({ ...formData, variants: n });
                        }}
                      />
                      <input
                        className='col-span-2 p-3 rounded-xl border border-slate-200 text-xs text-center focus:bg-white'
                        placeholder='🍏'
                        value={v.emoji}
                        onChange={(e) => {
                          const n = [...formData.variants];
                      ...
                      <input
                        required
                        type='number'
                        className='col-span-4 p-3 rounded-xl border border-slate-200 text-xs focus:bg-white'
                        placeholder='Stock'
                        value={v.stock}
                        onChange={(e) => {
                          const n = [...formData.variants];
