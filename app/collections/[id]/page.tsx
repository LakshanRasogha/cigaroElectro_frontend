          ...
          >
            <div className="bg-gradient-to-br from-zinc-900 to-black backdrop-blur-2xl text-white p-5 rounded-[2rem] shadow-2xl flex items-center justify-between border border-[#D4AF37]/30">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-[10px] font-black uppercase tracking-widest text-[#D4AF37]">Assigned</p>
                  <p className="text-sm font-bold">{currentVariant?.flavor}</p>
                </div>
              </div>
              <button onClick={() => router.push('/cart')} className="px-6 py-3 bg-gradient-to-r from-[#D4AF37] to-[#B49450] text-black rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">
                View Bag
              </button>
            </div>
          </motion.div>
      ...
      <div className="relative z-10 pt-32 pb-20 px-4 sm:px-6 lg:px-10">
        <section className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center gap-3 text-zinc-600 mb-12">
            <button onClick={() => router.push('/collections')} className="hover:text-[#D4AF37] transition-colors text-[10px] font-bold uppercase tracking-widest">Catalog</button>
            <ChevronRight size={14} />
            <span className="text-[#D4AF37] font-black uppercase tracking-widest text-[10px]">{product.category}</span>
            <ChevronRight size={14} />
            <span className="text-white font-bold text-[10px] uppercase tracking-widest">{product.name}</span>
          </div>
            ...
            <div className="space-y-8">
              <div className="relative sticky top-32">
                <AnimatePresence mode="wait">
                  ...
                  >
                    <img src={gallery[selectedImage] || product.productImage?.[0]} className="w-full h-full object-cover" alt={product.name} />
                    <div className="absolute top-8 right-8 flex flex-col gap-4">
                      <button onClick={() => setIsWishlisted(!isWishlisted)} className="p-4 bg-black/60 backdrop-blur-xl rounded-2xl border border-white/10">
                        <Heart size={20} className={isWishlisted ? "fill-[#D4AF37] text-[#D4AF37]" : "text-white/70"} />
                      </button>
                    </div>
                  </motion.div>
                </AnimatePresence>
              ...
              <div className="flex gap-4 overflow-x-auto pb-4 no-scrollbar">
                {gallery.map((img: string, idx: number) => (
                  <button key={idx} onClick={() => setSelectedImage(idx)} className={`min-w-[110px] h-[110px] rounded-3xl overflow-hidden border-2 transition-all ${selectedImage === idx ? 'border-[#D4AF37]' : 'border-transparent opacity-40'}`}>
            ...
            <div className="space-y-12">
              <div className="space-y-6">
                <span className="px-5 py-2.5 bg-[#D4AF37]/10 text-[#D4AF37] rounded-full text-[10px] font-black uppercase tracking-[0.4em] inline-flex items-center gap-3 border border-[#D4AF37]/20">
                  <Zap size={14} className="fill-current" /> {product.category} Series
                </span>
                <h1 className="text-5xl lg:text-7xl xl:text-8xl font-black text-white tracking-tighter leading-[0.85]">
                  {product.name}
                </h1>
                <p className="text-xl text-zinc-400 font-light italic">"{product.tagline}"</p>
                <p className="text-base text-zinc-500 leading-relaxed max-w-xl">{product.description}</p>
              </div>
              ...
              <div className="grid grid-cols-3 gap-8 py-10 border-y border-white/5">
                {[
                  ...
                  <div key={i} className="text-center">
                    <spec.icon className="mx-auto text-[#D4AF37] mb-3" size={20} />
              ...
              <div className="p-12 rounded-[3.5rem] bg-gradient-to-br from-white/[0.03] to-white/[0.01] backdrop-blur-3xl border border-white/10 shadow-2xl relative overflow-hidden">
                <div className="relative z-10 flex justify-between items-center">
                  <div>
                    <p className="text-[#D4AF37] text-[10px] font-black uppercase tracking-[0.5em] mb-2">Unit Valuation</p>
                    <span className="text-4xl lg:text-6xl font-black text-white tracking-tighter">Rs. {product.basePrice?.toLocaleString()}</span>
                  </div>
                  <div className="text-right">
                    <p className="text-zinc-600 text-[10px] font-black uppercase tracking-widest mb-1">Delivery</p>
                    <p className="font-black text-2xl text-white">Rs. {product.deliveryFee?.toLocaleString()}</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-12">
            <div className="border-b border-white/5 pb-8 flex items-end justify-between">
              <h2 className="text-4xl lg:text-6xl font-black text-white tracking-tighter uppercase">Editions<span className="text-[#D4AF37]">.</span></h2>
              <div className="flex items-center gap-2 text-zinc-500">
                <Package size={20} className="text-[#D4AF37]" />
                <span className="text-xs font-black uppercase tracking-widest">{product.variants?.length} Variants Available</span>
              </div>
            </div>
        ...
        <div className="space-y-6">
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-4 text-zinc-400">
               <span className="text-xs font-bold uppercase tracking-widest">{variant.nicotine || '5% Strength'}</span>
               <div className="w-1.5 h-1.5 rounded-full bg-[#D4AF37]" />
               <span className="text-xs font-bold uppercase tracking-widest">{variant.stock} Units Left</span>
            </div>
          </div>
...
const MobileVariantCard = ({ variant, isActive, onSelect, onAddToCart }: any) => {
  const [isAdded, setIsAdded] = useState(false);
  ...
  return (
    ...
    >
      <div className="absolute inset-0">
        <img src={variant.variantImage?.[0]} className="w-full h-full object-cover opacity-60" alt={variant.flavor} />
        <div className="absolute inset-0 bg-gradient-to-t from-black to-transparent" />
      </div>

      <div className="relative z-10 h-full p-3 flex flex-col justify-between">
        <div className="w-8 h-8 bg-black/40 backdrop-blur-md rounded-lg flex items-center justify-center text-lg">{variant.emoji}</div>
        
        <div className="space-y-2">
          <h3 className="text-[10px] font-black text-white uppercase truncate">{variant.flavor}</h3>
          <button
            onClick={(e) => {
              e.stopPropagation();
              onAddToCart(variant);
