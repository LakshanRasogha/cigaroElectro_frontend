...
import { 
  User, Mail, Lock, Phone, MapPin, Zap, ArrowRight, 
  Loader2, Globe, Sparkles, ShieldCheck, Crown, Gem 
} from 'lucide-react';
      ...
      >
        ...
        <div className="text-center mb-6 sm:mb-8 md:mb-10">
          ...
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-black text-white tracking-tighter flex items-center justify-center gap-2">
            Join the <span className="italic text-transparent bg-clip-text bg-gradient-to-r from-[#D4AF37] via-[#F2D37D] to-[#AA771C]">Circle.</span>
        ...
        <form className="space-y-4 sm:space-y-5 md:space-y-6" onSubmit={handleSubmit}>
          ...
          <p className="text-center text-[7px] sm:text-[8px] md:text-[9px] lg:text-[10px] font-black uppercase tracking-[0.15em] sm:tracking-[0.2em] text-zinc-600 mt-4 sm:mt-6 md:mt-8">
            Established member?{' '}
            <Link href="/auth/login" className="text-[#D4AF37] hover:text-[#F2D37D] transition-colors">
              Access Terminal
            </Link>
          </p>
...
const InputField = ({ icon, iconSm, label, name, type, value, onChange }: any) => (
  <div className="space-y-1 sm:space-y-1.5 md:space-y-2 w-full">
    <label className="text-[7px] sm:text-[8px] md:text-[9px] font-black uppercase tracking-[0.1em] sm:tracking-[0.15em] md:tracking-[0.2em] text-zinc-600 ml-2 sm:ml-3 md:ml-4 flex items-center gap-1 sm:gap-2">
    ...
    <input 
      ...
      className="w-full bg-white/[0.03] border border-white/5 rounded-lg sm:rounded-xl md:rounded-2xl px-3 sm:px-4 md:px-5 lg:px-6 py-2.5 sm:py-3 md:py-4 text-white text-xs sm:text-sm font-bold placeholder:text-zinc-800 focus:bg-white/[0.05] focus:border-[#D4AF37]/30 focus:ring-2 focus:ring-[#D4AF37]/10 transition-all outline-none"
    />
  </div>
);
